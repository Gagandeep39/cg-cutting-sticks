...
public class CuttingStick {
	...
	public void findMinCost(int stickLength, List<Integer> indices) {
		...
		for (int i = 0; i < indices.size(); i++) {
			recurse(stickLength, indices, i, new boolean[indices.size()], new ArrayList<Integer>(), result);
		}
		System.out.println(result);
	}

	private void recurse(int stickLength, List<Integer> indices, int curIndex, boolean used[], List<Integer> curOrder,
			Result result) {
		used[curIndex] = true;
	...
	public static void main(String args[]) {
		CuttingStick sp = new CuttingStick();

		sp.findMinCost(100, Arrays.asList(new Integer[] { 25, 50, 75 }));
		// 10 (4,6) + 4(2,2) + 6(3,3)

		sp.findMinCost(4, Arrays.asList(new Integer[] { 4, 5, 7, 8 }));
